...
from [`src/index.js`](popup/src/scripts/index.js).

If you had previously enabled service workers in your production deployment and
have decided that you would like to disable them for all your existing users,
you can swap out the call to `registerServiceWorker()` in
[`src/index.js`](popup/src/scripts/index.js) first by modifying the service worker import:
```javascript
...
When users install your app to the homescreen of their device the default configuration will make a shortcut to `/index.html`. This may not work for client-side routers which expect the app to be served from `/`. Edit the web app manifest at [`public/manifest.json`](manifest.json) and change `start_url` to match the required URL scheme, for example:
